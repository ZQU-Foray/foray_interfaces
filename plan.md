...
- [ ] **周期与调度归链路实现**（`foray_platform` 与 `ControllerCode`）——本仓不再定义「多久发一次」，
      消息表里也没有这一列。由此产生一个跨组义务：**电控组那侧按什么速率预期收包、看门狗设多少**，
      需与 `foray_platform` 对齐。这与帧格式那次是**同形**的跨组问题（契约落在实现仓）
- [ ] **超时与失效阈值同样不在本仓**（见上）——它与周期本是一对数字，现在两者都在链路侧，
      但**没有任何地方承载**。需确认由谁给它们一个权威落点
